...
export default function Navbar() {
  ...
  return (
    <nav id='navbar' className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-lg' : 'bg-transparent'} border-b border-gray-200`}>
      ...
      <div className={`fixed top-0 left-0 w-64 h-full bg-white text-gray-900 z-40 transform transition-all duration-300 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} border-r border-gray-200`}>
        ...
        <div className="flex flex-col items-center space-y-8 py-10">
          ...
          <button onClick={() => handleMenuClick('about')} className="hover:text-blue-600 text-gray-900 transition-colors">About</button>
